 ...
 */
@Service
public class OrderRefundedAnalyzer extends IStatAnalyzerHelper {
    @Override
    public StatType getStatType() {
        return StatType.ORDER_REFUNDED;
    }

    @Override
    protected String getSizeOfTotalSql(Platform platform, String retailerUuid) {
        String sql = "select count(DISTINCT(order_no)) as size from o_order_item where state in ('WITH_WAIT_SHIP_REFUNDED','WITH_RECEIVED_REFUNDED') ";
    ...
    protected String getSizeOfDayBetweenSql(Platform platform, String retailerUuid, String dayStart, String dayEnd) {
        String sql = "select count(DISTINCT(order_no)) as size from o_order_item where state in ('WITH_WAIT_SHIP_REFUNDED','WITH_RECEIVED_REFUNDED') and refund_at >= '" + dayStart + "' and " + "refund_at <= '" + dayEnd + "'";
