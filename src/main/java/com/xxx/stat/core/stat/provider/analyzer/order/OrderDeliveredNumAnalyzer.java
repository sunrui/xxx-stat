 ...
 */
@Service
public class OrderDeliveredNumAnalyzer extends IStatAnalyzerHelper {
    @Override
    public StatType getStatType() {
        return StatType.ORDER_DELIVERED_NUM;
    }

    @Override
    protected String getSizeOfTotalSql(Platform platform, String retailerUuid) {
        String sql = "select count(1) as size from o_order where  state =  'DELIVERED'  ";
    ...
    protected String getSizeOfDayBetweenSql(Platform platform, String retailerUuid, String dayStart, String dayEnd) {
        ...
        if (StringUtils.isEmpty(retailerUuid)) {
            sql = "select count(id) as size from o_order where  state =  'DELIVERED'  and created_at >= '" + dayStart + "' and " + "created_at <= '" + dayEnd + "'";
        } else {
            sql = "select  count(id) as size from o_order_item where  state =  'DELIVERED'  and created_at >= '" + dayStart + "' and " + "created_at <= '" + dayEnd + "'";
        }
